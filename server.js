...
// ══════════════════════════════════════════════════
app.get('/', (req, res) => {
  res.json({ status: 'RevAnalysis worker running' });
});

// ══════════════════════════════════════════════════
//  GENERATE ENDPOINT
//  Called by Vercel queue-report.js
//  Returns 200 immediately, generates in background
// ══════════════════════════════════════════════════
app.post('/generate', (req, res) => {
  ...
  generateAndSend({ email, bizName, industry, calcData, answers })
    .then(() => console.log(`✓ Report delivered to ${email}`))
    .catch(e => console.error(`✗ Report failed for ${email}:`, e.message));
});
...
async function generateAndSend({ email, bizName, industry, calcData, answers }) {
  ...
  for (let i = 0; i < SECTION_KEYS.length; i++) {
    ...
    console.log(`Generating section ${i+1}/${SECTION_KEYS.length}: ${key}`);

    try {
      const prompt = buildSectionPrompt(key, ctx);
      const result = await callAnthropic(prompt);
      const parsed = parseSecs(result);
      sections[key] = parsed[key] || Object.values(parsed)[0] || getFallbackSection(key, calcData, bizName, industry);
    } catch(e) {
      console.warn(`Section ${key} failed, using fallback:`, e.message);
      sections[key] = getFallbackSection(key, calcData, bizName, industry);
    }

    // 22-second delay between calls — stays under Anthropic rate limit
    if (i < SECTION_KEYS.length - 1) {
      await sleep(22000);
    }
  }

  console.log('All sections generated. Building PDF...');
  ...
  try {
    ...
    console.warn('PDF generation failed, sending without attachment:', e.message);
  }
...
function getFallbackSection(key, calcData, bizName, industry) {
  const L = calcData;
  const map = {
    EXEC: `<p>Our diagnostic estimates approximately $${L.total.toLocaleString()} in annual revenue opportunity for ${bizName} (conservative range: $${L.totalLo.toLocaleString()}–$${L.totalHi.toLocaleString()}).</p><div class="disclaimer">All figures are estimates based on ranges you provided.</div>`,
    CONV: `<h4>Close Rate Analysis</h4><p>At ~${Math.round(L.meta.close*100)}%, improving toward the 65% benchmark represents a significant estimated annual opportunity.</p>`,
    DEAD: `<h4>Value in Pipeline</h4><p>~${L.meta.dead} leads × $${L.meta.avgLo.toLocaleString()} avg × 12% re-engagement = approximately $${Math.round(L.meta.dead*L.meta.avgLo*0.12).toLocaleString()} estimated recoverable revenue.</p>`,
    MKTG: `<p>For ${industry}, Google Business Profile and referral activation are the highest-ROI channels.</p>`,
    RET: `<p>Bain & Company: 5% retention improvement drives 25–95% profit growth. At ~${L.meta.annCusts} annual customers, even modest improvement has material annual impact.</p>`,
    REF: `<p>Texas Tech: 83% of satisfied customers would refer — only 29% do because they're never asked.</p>`,
    PRICE: `<p>McKinsey: 1% price improvement = ~11% profit improvement. A conservative 6% adjustment adds ~$${Math.round(L.meta.revLo*0.06).toLocaleString()} annually.</p>`,
    REV: `<p>BrightLocal: 93% of consumers check reviews. Implement a systematic post-job review request within 24–48 hours.</p>`,
    OPS: `<p>Each complaint costs 4–6x the transaction value. At $${L.meta.avgLo.toLocaleString()} average, each avoidable complaint costs ~$${Math.round(L.meta.avgLo*4).toLocaleString()}.</p>`,
    PRIORITY: `<h4>Priority Rankings for ${bizName}</h4><table><tr><th>Rank</th><th>Category</th><th>Est. Opportunity</th><th>90-Day Target</th></tr>${L.cats.map((c,i)=>`<tr><td>#${i+1}</td><td>${c.n}</td><td>~$${c.amt.toLocaleString()}</td><td>$${Math.round(c.amt*0.15).toLocaleString()}–$${Math.round(c.amt*0.25).toLocaleString()}</td></tr>`).join('')}</table>`,
    PLAN: `<p>Focus the first 30 days on: lead follow-up system, re-engaging dormant leads, review request process.</p>`,
    ROI: `<h4>Conservative Recovery Projection</h4><p>At the realistic 22% recovery scenario: ~$${Math.round(L.total*0.22).toLocaleString()} in 90-day recovery on a $297 investment — approximately ${Math.round(L.total*0.22/297)}x return.</p>`,
  };
  return map[key] || `<p>Section unavailable. Please contact support.</p>`;
}
